...
def getConcept():
    ...
    listConcept.append((r"Đoạn\sthẳng\s+([A-Z]{2})", "\\g<1>"))
    ...
    listConcept.append(
        (r"(Tam giác)\s+(vuông|cân|vuông\scân|đều)\s+([A-Z]{3})", formatTriangleWithAttributes))
    listConcept.append(
        (r"(Hình\s+thang(?:\s+(?:cân|vuông))?)\s+([A-Z]{4})", formatSecondUpper))
    listConcept.append(
        (r"(Hình\s+(?:bình\s+hành|thoi|vuông|chữ\s+nhật))\s+([A-Z]{4})", formatSecondUpper))
    listConcept.append(
        (r"Đường\s+tròn(?:\s+tâm)?\s+([A-Z])(?:(?:\s+bán\s+kính\s+)|\,)?([A-Z])", formatCircle))
    return listConcept
